...
// 仕様書 表2.3 のADSR値を定数化（mainEnv用・暫定値）
...
// 1音分のクラリネット音色を作るクラス（仕様書どおり3系統ADSR: mainEnv / reedEnv / breathEnv）
class ClarinetInstrument implements Instrument {
  ADSR mainEnv;              // メイン波形の音量エンベロープ（仕様書のmainEnv）
  ADSR reedEnv;              // リードノイズの音量エンベロープ（仕様書のreedEnv）
  ADSR breathEnv;            // ブレスノイズ（低域空気感）の音量エンベロープ（仕様書のbreathEnv）

  ClarinetInstrument(float freq, float amp) {            // 引数:周波数(Hz), 振幅(0.0〜1.0)
    // FFT実測（仕様書 表2.4）に基づく倍音テーブル（第16倍音まで。奇数倍音が支配的でh3>h1）
    float[] harmonicAmp = {
    ...
    // トーン用LPF: 仕様書どおりカットオフを音程(ノート周波数)に連動させる（freq*18・上限12000Hz）
    MoogFilter lpf = new MoogFilter(min(12000.0f, freq * 18.0f), 0.0f, MoogFilter.Type.LP);
    osc.patch(lpf);                                       // 発振器 → ローパスフィルタへ送る
    mainEnv = new ADSR(amp, ATTACK, 0.040f, SUSTAIN, RELEASE); // メイン音の音量エンベロープ（仕様書 表2.3）
    lpf.patch(mainEnv);                                  // フィルタ後の信号 → エンベロープへ送る

    // リードノイズ: 仕様書どおり 1200〜2800Hz に絞ったホワイトノイズでリードの擦れ感を出す
    Noise      noise = new Noise(0.0003f, Noise.Tint.WHITE);        // 非常に小さなホワイトノイズを発生させる
    ...
    reedEnv = new ADSR(0.0003f, 0.010f, 0.130f, 0.05f, 0.080f); // リード用ADSR（仕様書: A0.010,S0.05,R0.080）
    nlpf.patch(reedEnv);                                 // 帯域制限したノイズ → エンベロープへ送る

    // ブレスノイズ: 仕様書どおり 80Hz以下のピンクノイズで息のふくらみを出す
    ...
    breathEnv = new ADSR(0.002f, ATTACK, 0.040f, 0.6f, RELEASE); // ブレス用ADSR（仕様書: A/RはmainEnvと同じ）
...
void draw() {                                            // 毎フレーム繰り返し実行される描画処理
  ...
  text("BPM: 100  |  きらきら星 (仕様書要素まで)", 20, 70); // 状態を画面に表示する
}
